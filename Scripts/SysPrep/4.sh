...
if [ -e .vmfusion_version ] || [[ "$PACKER_BUILDER_TYPE" == vmware* ]]; then
    ...
    # This usually fails
    # This could be for many reasons, but most likely there is a lingering process access this mount.
    # It shouldn't cause any issues and should go away when we reboot the VM.
